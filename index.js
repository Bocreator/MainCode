...
import voice, { joinVoiceChannel, createAudioPlayer, createAudioResource, AudioPlayerStatus } from "@discordjs/voice";
...
const discClient = new DiscordJS.Client({ 
    intents: [
            ...
            if(args[0].replace(prefix, "") === "play")
            {
                ...
                if(args.length >= 2)
                {
                    ...
                    else
                    {
                        ...
                        const video = await videoFinder(args.join(" ").replace(`${prefix}play`, ""))
